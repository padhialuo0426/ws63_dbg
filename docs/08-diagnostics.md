...
下文使用 [路径设置](01-preparation.md#11-设置工程与-sdk-路径) 中的两个变量。
...
它能从身份匹配的 ELF 补充捕获时已经核对过的可执行 Flash 内容；其他未捕获区域仍不可读。
写寄存器、写内存、断点、复位和继续执行均被拒绝；离线分析不会打开 USB 探针。

## 8.4 明确访问外设寄存器

GDB 普通内存写入可能对非对齐地址做读改写。对有访问副作用的 MMIO，使用明确的 32 位访问：

```gdb
monitor md 0x48000210 1
```

本板这是 SFC 的 Flash 大小配置寄存器，返回一个地址和值。`monitor mw 地址 值` 只写一次，
不自动读回；地址必须四字节对齐。读清、写一清零和 FIFO 寄存器须按 SDK 定义访问，
当前没有自动刷新寄存器面板或 SVD 语义解析。
